...
module.exports = {
    name: 'partnerreq',
    description: 'the partnerreq embed',
    ...
    async run(interaction, client, extras) {
        let pathtxt = path.join(__dirname, '../../../text docs/partner.txt');
        var text = fs.readFileSync(pathtxt).toString('utf-8');

        const embed = new EmbedBuilder().setDescription(text).setTitle('How To Partner').setColor("Blurple");
